...
// Spectral shape of the ASBC Tristimulus method, 380-780 nm in 5 nm intervals
static const std::vector<double> LAMBDA = {
...
static const double DELTA_LAMBDA = 5.0;

// D65 illuminant, adjusted to spectral shape above
static const std::vector<double> I = {
...
static const std::vector<double> X_BAR = {
...
static const std::vector<double> Y_BAR = {
...
static const std::vector<double> Z_BAR = {
...
static double summate(const double& n, const std::vector<double>& cmf, const std::vector<double>& s) {
    double sum = 0.0;
...
static double correctGamma(double l) {
    if (l <= 0.0031308)
        l = l * 12.92;
    else
        l = 1.055 * pow(l, 1.0 / 2.4) - 0.055;
    return std::max(0.0, std::min(1.0, l));
}

static RGB beerSDToSRGB(const double& a430, const double& pathCM) {
    // Calculate the transmission spectrum distribution from the average of the absorption spectra of the ensemble of 99 beers
    std::vector<double> s = std::vector<double>(LAMBDA.size());
    for (int i = 0; i < LAMBDA.size(); ++i)
        s[i] = pow(10.0, -a430 * (0.02465 * exp(-(LAMBDA[i] - 430.0) / 17.591) + 0.97535 * exp(-(LAMBDA[i] - 430.0) / 82.122)) * pathCM);

    // Transform the spectrum distribution into the CIE XYZ color space with integration over 5 nm intervals
    double n = 0.0;
    for (size_t i = 0; i < Y_BAR.size(); ++i)
        n += Y_BAR[i] * I[i] * DELTA_LAMBDA;
    double x = summate(n, X_BAR, s);
    double y = summate(n, Y_BAR, s);
    double z = summate(n, Z_BAR, s);

    // Transform color space to sRGB
    double r = correctGamma(x * 3.2406 + y * -1.5372 + z * -0.4986);
    double g = correctGamma(x * -0.9689 + y * 1.8758 + z * 0.0415);
    double b = correctGamma(x * 0.0557 + y * -0.2040 + z * 1.0570);
    return { r, g, b };
}
