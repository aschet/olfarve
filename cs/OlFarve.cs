...
class OlFarve
{
    ...
    private static readonly float K = CalcScaleFactor();
    ...
    public static float[] SRMToSRGB(float srm, float pathCM = DEFAULT_PATH)
    {
    ...
    public static float[] EBCToSRGB(float ebc, float pathCM = DEFAULT_PATH)
    {
    ...
    public static string RGBToHex(float[] rgb)
    {
        ...
        for (int i = 0; i < rgb.Length; ++i)
            text += Convert.ToInt32(Math.Round(rgb[i] * 255.0f)).ToString("x2");
        return text;
    }
}
