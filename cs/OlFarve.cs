...
class OlFarve
{
    ...
    private static readonly double[,] CIE_DATA =
    {
    ...
    {
        double k = 0.0;
        for (int i = 0; i < CIE_DATA.GetLength(0); ++i)
            k += CIE_DATA[i, 3] * CIE_DATA[i, 1];
        return 1.0 / k;
    }

    private static readonly double K = CalcScaleFactor();
    ...
    private static double[] BeerSDToSRGB(double a430, double l)
    {
        ...
        for (int i = 0; i < CIE_DATA.GetLength(0); ++i)
        {
            double t = Math.Pow(10.0, -a430 * l * (0.02465 * Math.Exp(-(w - 430.0) / 17.591) + 0.97535 * Math.Exp(-(w - 430.0) / 82.122)));
            double d65 = CIE_DATA[i, 3];
