...
// Copyright 2024 Thomas Ascher <thomas.ascher@gmx.at>
...
func calcScaleFactor() float64 {
	k := 0.0
	for _, i := range CIE_DATA {
		k += i[3] * i[1]
	}
	return 1.0 / k
}

var K = calcScaleFactor()
...
func transferColorComponent(t float64) float64 {
...
func beerSdToSrgb(a430, l float64) []float64 {
	x := 0.0
	y := 0.0
	z := 0.0
	w := 380.0
	...
	r := transferColorComponent(x*3.2406255 + y*-1.537208 + z*-0.4986286)
	g := transferColorComponent(x*-0.9689307 + y*1.8757561 + z*0.0415175)
	b := transferColorComponent(x*0.0557101 + y*-0.2040211 + z*1.0569959)
...
func srmToSrgb(srm, pathCm float64) []float64 {
...
func ebcToSrgb(ebc, pathCm float64) []float64 {
...
func rgbToHex(rgb []float64) string {
	text := "#"
	for _, i := range rgb {
		text += fmt.Sprintf("%02x", int(math.Round(i*255.0)))
	}
	return text
}

func main() {
	fmt.Println("SRM,sRGB")
	for i := 1; i <= 50; i++ {
		fmt.Printf("%d,%s\n", i, rgbToHex(srmToSrgb(float64(i), DEFAULT_PATH)))
	}
}
