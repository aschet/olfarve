...
public final class OlFavre {
	...
	private static final float[][] CIE_DATA = {
	...
	private static final float K = calcScaleFactor();
	...
	public static final float DEFAULT_PATH = 5.0f;
	...
	public static float[] srmToSRGB(float srm, float pathCM) {
	...
	public static float[] ebcToSRGB(float ebc, float pathCM) {
	...
	public static String rgbToHex(float[] rgb) {
		...
		for (int i = 0; i < rgb.length; ++i)
			text += String.format("%02x", Math.round(rgb[i] * 255.0f));
		return text;
	}
}
